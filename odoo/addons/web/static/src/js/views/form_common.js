odoo.define('web.form_common', function (require) {
    ...
    var CompletionFieldMixin = {
        ...
        get_search_result: function (search_val) {
            ...
            return this.orderer.add(dataset.name_search(search_val,
                new data.CompoundDomain(self.build_domain(), exclusion_domain),
                    ...
                    values.push({
                        ...
                        action: function () {
                            dataset.name_search(search_val, self.build_domain(), 'ilike', 160).done(function (_data) {
                                self._search_create_popup("search", _data, self._search_context(search_val));
                            });
                        },
        ...
        _search_context: function (search_val) {
            var tmp = {};
            if (this.field.relation === 'goods' && search_val) {
                if (search_val.match(/^[0-9a-zA-Z_-]+$/)) {
                    tmp['search_default_pinyin_abbr'] = search_val;
                } else {
                    tmp['search_default_name'] = search_val;
                }
            }
            return tmp;
        },
        // quick create set default name=value
    ...
    */
    ...
    var SelectCreateDialog = ViewDialog.extend({
        ...
        on_view_list_loaded: function () {
            this.$('input.o_searchview_input').focus();
        },
    });
                ...
                if (this.view_list.grouped) {
                    group_domain = _.chain(_.values(this.view_list.groups.children))
                        .filter(function (child) {
                            return child.records.length;
                        })
                        .map(function (c) {
                            return c.datagroup.domain;
                        })
                        .value();
