        ...
        edit: function (record, configureField) {
            ...
            return loaded.then(function () {
                ...
                _(self.form.fields).each(function (field, name) {
                    ...
                    field.$el.on('focus', self._auto_focus);
                });
                return self.form;
            });
        },
        _auto_focus: function (e) {
...
// TODO: not sure second @lends on existing item is correct, to check
    ListView.include(/** @lends instance.web.ListView# */{
        ...
        bind_events: function () {
            var self = this;
            this.on('list_view_loaded', this, function (event) {
                self.$('div.table-responsive').on('scroll', _.throttle(this.on_scroll.bind(this), 30, {leading: false}));
            });
        ...
        on_scroll: function () {
            if (!this.editable() || this.editor.is_editing()) {
                this.resize_fields();
            }
        },
        destroy: function () {
        ...
        _key_move_record: function (event, record_direction, is_valid_move) {
            if (!this.editor.is_editing() || this.editor.is_creating()) {
                return $.when();
            }
        ...
        keydown_LEFT: function (e) {
            ...
            while (--field_index >= 0) {
                ...
                if (this._is_field_required(field)) {
                    break;
                }
            }
        ...
        _is_field_required: function (field) {
            return field && field.$el.is(':visible:not(.o_readonly)') && field.$el.hasClass('o_form_required');
        },
        _move_right: function (e) {
            ...
            var fields = this.editor.form.fields;
            var field;
            while (++field_index < fields_order.length) {
                field = fields[fields_order[field_index]];
                if (this._is_field_required(field)) {
                    break;
                }
            }
            if (field && field_index < fields_order.length) {
                field.focus();
            }
            return $.when();
        },
        _check_next: function (e) { // Keydown and not keyup because this handler must be called before the browser has focused the next field
            var form = this.editor.form;
            var required_fields = _(form.fields_order).chain()
                .map(function (name) {
                    return form.fields[name];
                })
                .filter(this._is_field_required);
            var last_required_field = required_fields.last().value();

            var last_editable_fields = _(form.fields_order).chain()
            .map(function (name) { return form.fields[name]; })
            .filter(function (field) { return field && field.$el.is(':visible:not(.o_readonly)'); });
            var last_field = last_editable_fields.last().value();

            // tabbed from last field in form
            if ((last_required_field && $(e.target).closest(last_required_field.el).length) ||
                (last_field && $(e.target).closest(last_field.el).length)) {
                e.preventDefault();
                return this._next();
            } else {
                e.preventDefault();
